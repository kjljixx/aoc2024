...
void getAntinodesFromPair(std::pair<int, int>& a, std::pair<int, int>& b,
  ...
  if(validCoord({a.first + movement.first, a.second + movement.second}, size)){
    // std::cerr << a.first + movement.first << " " << a.second + movement.second << " "<< antinodes.size() << std::endl;
    antinodes.insert({a.first + movement.first, a.second + movement.second});
  }
  if(validCoord({b.first - movement.first, b.second - movement.second}, size)){
    // std::cerr << b.first - movement.first << " " << b.second - movement.second << " "<< antinodes.size() << std::endl;
    antinodes.insert({b.first - movement.first, b.second - movement.second});
  }
}
